          ...
          <CardContent>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-sm font-medium">
                {accountData.monthlyBudget.amount} /{" "}
                {accountData.monthlyBudget.total}
              </div>
            </div>
            <Progress
              value={accountData.monthlyBudget.spent}
              className="h-2 mb-2"
            />
                ...
                {accountData.cards.map((card, index) => (
                  <div
                    ...
                    className={`snap-center shrink-0 ${
                      index === activeCard ? "scale-100" : "scale-95 opacity-70"
                    }`}
                ...
                {accountData.cards.map((_, index) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full ${
                      index === activeCard ? "bg-primary" : "bg-gray-300"
                    }`}
                    onClick={() => setActiveCard(index)}
                  />
                ))}
          ...
          <CardContent>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-2xl font-bold">
                {accountData.investments.totalValue}
              </h3>
              <Button variant="outline" size="sm">
                Invest More
              </Button>
              ...
              <div className="flex flex-wrap mt-2">
                {accountData.investments.allocation.map((asset, index) => (
                  <div key={index} className="flex items-center mr-4 mt-1">
                    <div
                      className={`w-3 h-3 rounded-full ${asset.color} mr-1`}
                    ></div>
